...
//TODO:HEre.
	 DEBUG_PRINTF4("CaseA: Socket %d received %d bytes: '%s'\n",*socket,*numbytes,buf);

	 if (send(*socket,buf,*numbytes,0)==-1){
	 		 perror("SubscriberInformationRequestAction: send");
	 	 }
	 DEBUG_PRINTF4("CaseA: Socket %d SEND %d bytes: '%s'\n",*socket,*numbytes,buf);
 ...
 int CaseC(int* socket, char buf[], int *numbytes)
{
 ...
 int ClientConnectionInit(int* sockfd, char ip[], char port[]){
 	...
 	inet_ntop(p->ai_family, get_in_addr((struct sockaddr *)p->ai_addr),
 			s, sizeof s);
 	DEBUG_PRINTF3("ClientConnectionInit: connecting to ip %s in port %s\n", s,port );
